...
/// A leonardo process represents an actor.
pub struct Process {
    task: Task<Result<()>>,
}
        ...
        A::Wrap: Send,
    {
        ...
        let mut process = AsyncWormhole::new(stack, move |yielder| {
            ...
            match module.runtime() {
                ...
                Runtime::Wasmtime => {
                    let mut linker = WasmtimeleonardoLinker::new(module, yielder_ptr, memory)?;
                    ...
                    match function {
                        ...
                        FunctionLookup::TableIndex(index) => {
                            let func =
                                instance.get_func("leonardo_spawn_by_index").ok_or_else(|| {
                                    anyhow::Error::msg(
                                        "No function leonardo_spawn_by_index in wasmtime instance",
                                    )
                                })?;
                ...
                Runtime::Wasmer => {
                    let mut linker = WasmerleonardoLinker::new(module, yielder_ptr, memory)?;
                    linker.add_api::<A>(api);
                    let instance = linker.instance()?;
                    ...
                    match function {
                        ...
                        FunctionLookup::TableIndex(index) => {
                            let func = instance.exports.get_function("leonardo_spawn_by_index")?;
                            func.call(&[(index as i32).into()])?;
                        }
                    }
